...
export default function PiercingGallery() {
  ...
  const [selectedPiercingValue, setSelectedPiercingValue] = useState()

  const { piercingGallery, loadMore } = usePiercingGallery();

  const handleOpen = (id) => {
    setOpen(!open)
    const selectedPiercing = piercingGallery.find(piercing => piercing.id === id)
    setSelectedPiercingValue(selectedPiercing)
  }
  ...
  return (
    <div className='flex flex-col justify-center items-center gap-16 xl:gap-28 lg:px-0 px-8 bg-black text-white h-auto xl:pt-48 xl:py-0 py-36 w-full mx-w-[800px]'>
        <h1 className={`${!open ? '' : 'brightness-50'} mx-auto lg:text-7xl text-5xl font-[Engraver]`}>Piercing Gallery</h1>
        <div className={`${!open ? '' : 'brightness-50'} grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-10 duration-300`}>
          {piercingGallery.map((piercing) => (
            <div key={piercing.id} onClick={() => handleOpen(piercing.id)} className='flex justify-center lg:justify-start items-center mx-auto lg:mx-0 border-[#3d3c3d] border-[10px] xl:h-[400px] xl:w-[350px] lg:h-auto lg:w-auto h-80 w-80 relative group overflow-hidden z-30'>
              {piercing.image 
              ? 
              <img src={piercing.image} alt={[piercing].id} className='xl:h-full xl:w-full object-cover lg:h-80 h-auto w-auto group-hover:brightness-75 cursor-pointer duration-300 group-hover:scale-105' /> 
              :
        ...
        <div className='relative'>
          <Dialog open={open} handler={handleOpen} className='fixed inset-0 flex flex-col items-center xl:py-14 xl:px-14 xl:h-[900px] h-auto w-auto max-w-max mx-auto justify-center my-auto outline-none bg-black xl:bg-[#3d3c3d]'>
            ...
            <DialogBody className='h-full w-auto items-center flex'>
              <img src={selectedPiercingValue?.image} className='xl:h-full h-max w-full'/>
            </DialogBody>
          </Dialog>
        </div>
      </div>
